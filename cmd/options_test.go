...
func TestOptionsEmpty(t *testing.T) {
	_, stdout, exitCode := withContext(t, []string{}, "", NewOptions)
	assert.NotEqual(t, -1, exitCode)
	assert.Contains(t, stdout, "try 'tennis --help'")
}

func TestOptionsBogus(t *testing.T) {
	_, stdout, exitCode := withContext(t, []string{"--bogus"}, "", NewOptions)
	assert.NotEqual(t, -1, exitCode)
	assert.Contains(t, stdout, "try 'tennis --help'")
}

func TestOptions(t *testing.T) {
	o, _, exitCode := withContext(t, []string{"-n", "--color=never", "--theme=light", "--width=100"}, "foo", NewOptions)
	...
	assert.Equal(t, 100, o.Table.TermWidth)
}
