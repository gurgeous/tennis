...
_tennis() {
  ...
  if [[ "${cur}" == -* ]]; then
    COMPREPLY=($(compgen -W "-n --row-numbers -t --title -r --reverse --border --color --delimiter --digits --theme --vanilla --width --select --sort --head --tail --completion --help --version " -- "${cur}"))
  else
    _filedir csv
